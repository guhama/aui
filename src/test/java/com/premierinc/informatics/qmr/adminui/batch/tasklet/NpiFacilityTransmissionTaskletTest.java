...
public class NpiFacilityTransmissionTaskletTest {

  @Mock
  private PackageHandlerAE packageHandler;

  @InjectMocks
  private NpiFacilityTransmissionTasklet npiFacilityTransmissionTasklet;
  ...
  public void testMergeSuccess() throws Exception {
    ...
    when(packageHandler.mergeNpiFacilityTransmission(Mockito.any(Long.class))).thenReturn(results);
    
    RepeatStatus repeatStatus = npiFacilityTransmissionTasklet.execute(null, chunkContext);
    
    assertSame(repeatStatus, RepeatStatus.FINISHED);
    Mockito.verify(packageHandler, Mockito.times(1)).mergeNpiFacilityTransmission(Mockito.any(Long.class));
  ...
  public void testMergeFailure() throws Exception {
    ...
    npiFacilityTransmissionTasklet.execute(null, chunkContext);
  }
